...
box off
cd figures
print -dpdf fig3a_upperleft.pdf
cd ..
...
box off
cd figures
print -dpdf fig3a_upperright.pdf
cd ..

%% Figure 3B

% Main
binsize = .1; % bin size to compute 2D histogram of two first PCs
bins = [-3.5:binsize:3.5]; % bins used
Hcs = zeros(length(bins)-1); % 2D Histogram
cx = nan(length(bins)-1,1); % bin centers
for i=1:length(bins)-1
    for j=1:length(bins)-1
        indx = pca1>=bins(i) & pca1<bins(i+1);
        indy = pca2>=bins(j) & pca2<bins(j+1);
        Hcs(i,j)=sum(indx & indy);
    end
    cx(i) = .5*(bins(i)+bins(i+1));
end
A=imgaussfilt(Hcs,5); % Gaussian filter on 2D histogram
figure('color','w','paperposition',[1 1 10 8])
contour(cx,cx,A,3,'k','linewidth',0.1,'fill','off');
hold on;
axis xy
lm = 2.5;
xlim([-lm lm])
...
set(gca,'xtick',[-round(lm):round(lm)],'ytick',[-round(lm):round(lm)])
box off
cd figures
...
% colors
cc = {[100 20 0]/100,[0 60 50]/100,[0 45 70]/100,[255 215 0]/255,[75 0 130]/255};
gg = .6*[1,1,1];
...
for v=1:5
    xlim([-lm lm])
    ylim([-lm lm])
    plot([0,w1(v)],[0,w2(v)],'color',cc{v},'linewidth',2); % vector of loadings for dilemma v
    hold on
    mm = w2(v)/w1(v); % direction of loadings
    plot([-lm,lm],[-mm*lm,mm*lm],'--','color',cc{v},'linewidth',.25); % projected dimension v
end
plot(xlim,[0,0],'k-');
plot([0,0],ylim,'k-');
xlim([-lm lm])
